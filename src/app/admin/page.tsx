...
export default function Admin() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    setCategories((await res.json()).results || []);
  }, []);

  const fetchProducts = useCallback(async () => {
    const url = selectedCategory === "all" ? "/api/products" : `/api/products?category_id=${selectedCategory}`;
    setProducts((await (await fetch(url)).json()).results || []);
  }, [selectedCategory]);

  const fetchBanners = useCallback(async () => {
    setBanners((await (await fetch("/api/banners")).json()).results || []);
  }, []);

  const fetchOrders = useCallback(async () => {
    const res = await fetch("/api/orders?admin=true", { credentials: "include" });
    setOrders((await res.json()).results || []);
  }, []);
